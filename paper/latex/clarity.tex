...
Applying the enhanced prompt to cleaned data degrades performance (0.831/0.797~$\rightarrow$~0.812/0.716 clarity Acc/F1), suggesting that aggressive preprocessing removes relevant signals such as fillers and hedging markers.
...
(2)~Prompt repair convergence is non-monotone, requiring best-of-$N$ selection without a principled stopping criterion.
(3)~Local baselines are limited; no full fine-tuning was evaluated.
(4)~Preprocessing the data did not translate into clear and consistent metric improvements in the setup.
...
Third, we plan to develop a best-iteration selection mechanism for the prompt enhancement pipeline.
...
\emph{Partial/half-answer} shows the lowest accuracy (13.3\%), though it is rarely predicted (15 instances).
...
for error analysis, and result aggregation across configurations.
